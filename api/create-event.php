...
try {
    $stmt = $pdo->prepare("INSERT INTO events (title, cover_image, cover_color, event_date, event_time, location, departments, description, registration_needed, registration_link, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
    $stmt->execute([
        $title,
        $cover_image_path,
        $cover_color,
